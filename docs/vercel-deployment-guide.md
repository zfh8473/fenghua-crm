# Vercel 部署指南

**日期：** 2026-01-14  
**项目：** fenghua-crm  
**目标：** 将项目部署到 Vercel 平台

---

## 📋 目录

1. [部署架构](#部署架构)
2. [前置要求](#前置要求)
3. [数据库和Redis配置](#数据库和redis配置)
4. [环境变量配置](#环境变量配置)
5. [前端部署步骤](#前端部署步骤)
6. [后端部署步骤](#后端部署步骤)
7. [验证部署](#验证部署)
8. [常见问题](#常见问题)

---

## 🏗️ 部署架构

### 架构概览

```
┌─────────────────┐
│   Vercel CDN    │  ← 前端静态资源
│  (fenghua-crm)  │
└────────┬────────┘
         │
         │ API 请求
         ▼
┌─────────────────┐
│ Vercel Functions│  ← 后端 Serverless Functions
│ (fenghua-backend)│
└────────┬────────┘
         │
         ├──► PostgreSQL (外部数据库)
         └──► Redis (外部Redis服务)
```

### 部署组件

1. **前端 (fenghua-frontend)**
   - 静态资源部署到 Vercel CDN
   - 使用 Vite 构建

2. **后端 (fenghua-backend)**
   - 部署为 Vercel Serverless Functions
   - 需要配置 `vercel.json` 文件

3. **外部服务**
   - PostgreSQL 数据库（推荐：Supabase、Neon、Railway）
   - Redis 服务（推荐：Upstash、Redis Cloud）

---

## ✅ 前置要求

### 1. 账户和工具

- ✅ Vercel 账户（免费版即可）
- ✅ GitHub 账户（用于代码仓库）
- ✅ PostgreSQL 数据库服务
- ✅ Redis 服务

### 2. 代码准备

- ✅ 代码已推送到 GitHub 仓库
- ✅ 所有依赖已添加到 `package.json`
- ✅ 代码已通过本地测试

---

## 🗄️ 数据库和Redis配置

### PostgreSQL 数据库

**推荐服务商：**

1. **Supabase** (推荐)
   - 免费版：500MB 数据库
   - 网址：https://supabase.com
   - 获取连接字符串：`Settings` → `Database` → `Connection string`

2. **Neon**
   - 免费版：3GB 数据库
   - 网址：https://neon.tech
   - 获取连接字符串：项目设置中

3. **Railway**
   - 免费版：$5 免费额度
   - 网址：https://railway.app
   - 获取连接字符串：数据库服务设置中

**连接字符串格式：**
```
postgresql://username:password@host:port/database?sslmode=require
```

### Redis 服务

**推荐服务商：**

1. **Upstash** (推荐，专为 Serverless 设计)
   - 免费版：10,000 命令/天
   - 网址：https://upstash.com
   - 获取连接字符串：创建数据库后获取

2. **Redis Cloud**
   - 免费版：30MB 内存
   - 网址：https://redis.com/cloud
   - 获取连接字符串：数据库配置中

**连接字符串格式：**
```
redis://default:password@host:port
```

---

## 🔐 环境变量配置

### 后端环境变量

在 Vercel 项目设置中配置以下环境变量：

#### 必需变量

| 变量名 | 说明 | 示例值 |
|--------|------|--------|
| `DATABASE_URL` | PostgreSQL 连接字符串 | `postgresql://user:pass@host:5432/db?sslmode=require` |
| `REDIS_URL` | Redis 连接字符串 | `redis://default:pass@host:6379` |
| `JWT_SECRET` | JWT 密钥（用于认证） | `your-super-secret-jwt-key-min-32-chars` |
| `DEPLOYMENT_PLATFORM` | 部署平台标识 | `vercel` |
| `VERCEL` | Vercel 环境标识 | `1` |
| `FRONTEND_URL` | 前端完整地址，用于 CORS（不设则登录等跨域请求被拦截） | `https://fenghua-crm-frontend.vercel.app` |

#### 可选变量

| 变量名 | 说明 | 默认值 |
|--------|------|--------|
| `PORT` | 服务端口 | `3001` |
| `JWT_EXPIRES_IN` | JWT 过期时间 | `7d` |
| `NODE_ENV` | 环境标识 | `production` |
| `PG_DATABASE_URL` | PostgreSQL 备用连接字符串 | 同 `DATABASE_URL` |

### 前端环境变量

在 Vercel 项目设置中配置以下环境变量：

#### 必需变量

| 变量名 | 说明 | 示例值 |
|--------|------|--------|
| `VITE_BACKEND_URL` | 后端 API 地址 | `https://your-backend.vercel.app` |

#### 可选变量

| 变量名 | 说明 | 默认值 |
|--------|------|--------|
| `VITE_API_BASE_URL` | API 基础地址（备用） | 同 `VITE_BACKEND_URL` |

---

## 🚀 前端部署步骤

### 步骤 1: 准备 Vercel 配置

在 `fenghua-frontend` 目录下创建 `vercel.json`（如果不存在）：

```json
{
  "buildCommand": "npm run build",
  "outputDirectory": "dist",
  "framework": "vite",
  "rewrites": [
    {
      "source": "/(.*)",
      "destination": "/index.html"
    }
  ]
}
```

### 步骤 2: 在 Vercel 中创建项目

1. 登录 [Vercel Dashboard](https://vercel.com/dashboard)
2. 点击 **"Add New Project"**
3. 选择 GitHub 仓库：`fenghua-crm`
4. 配置项目：
   - **Framework Preset**: `Vite`
   - **Root Directory**: `fenghua-frontend`
   - **Build Command**: `npm run build`
   - **Output Directory**: `dist`
   - **Install Command**: `npm install`

### 步骤 3: 配置环境变量

在项目设置中添加前端环境变量：
- `VITE_BACKEND_URL`

### 步骤 4: 部署

1. 点击 **"Deploy"**
2. 等待构建完成
3. 获取部署 URL（例如：`https://fenghua-crm.vercel.app`）

---

## 🔧 后端部署步骤

### 步骤 1: 后端路由与入口

- **Root Directory** 必须为 **fenghua-backend**。
- 项目内已包含：
  - **`api/index.js`**：Vercel Serverless 入口，将请求转发到 `dist/src/main` 的 Nest handler；
  - **`vercel.json`**：仅含 `rewrites`，把 `/(.*)` 转到 `/api`，使 `/health`、`/auth/login` 等能到达 Nest。
- **勿在 vercel.json 中使用 `builds`、`routes`**，以免触发 `fsPath` 等错误。超时在 **Settings → Functions → Max Duration** 中设置。

### 步骤 2: 入口与 Vercel 检测

- `fenghua-backend/src/main.ts` 在 `DEPLOYMENT_PLATFORM=vercel` 或 `VERCEL=1` 时导出 `handler`，供 `api/index.js` 调用；
- `npm run build` 产出 `dist/src/main.js`，由 `api/index.js` 通过 `require('../dist/src/main')` 引用。

### 步骤 3: 在 Vercel 中创建后端项目

1. 登录 [Vercel Dashboard](https://vercel.com/dashboard)
2. 点击 **"Add New Project"**
3. 选择 GitHub 仓库：`fenghua-crm`
4. 配置项目：
   - **Framework Preset**: `Other`
   - **Root Directory**: `fenghua-backend`（必填；留空会报 `No entrypoint found`，根目录无 `src/main.ts`）
   - **Build Command**: 留空（由 Vercel 零配置检测）；若失败可试 `npm run build`
   - **Output Directory**: 留空
   - **Install Command**: `npm install`

### 步骤 4: 配置环境变量

在项目设置中添加所有后端环境变量（见上方环境变量配置表）

### 步骤 5: 配置函数设置

在项目设置 → **Functions** 中：
- **Max Duration**: `60s`（在 **Settings → Functions** 中设置；零配置下无 vercel.json）
- **Memory**: `1024 MB`（推荐）

### 步骤 6: 部署

1. 点击 **"Deploy"**
2. 等待构建完成
3. 获取部署 URL（例如：`https://fenghua-backend.vercel.app`）

---

## ✅ 验证部署

### 1. 验证前端

1. 访问前端 URL：`https://fenghua-crm.vercel.app`
2. 检查页面是否正常加载
3. 尝试登录功能

### 2. 验证后端

1. 访问后端健康检查端点：
   ```
   GET https://fenghua-backend.vercel.app/health
   ```
2. 检查 API 响应

### 3. 验证数据库连接

1. 登录系统
2. 尝试创建/查看数据
3. 检查 Vercel 函数日志（在 Dashboard → Functions → Logs）

### 4. 验证 Redis 连接

1. 执行需要缓存的操作（如仪表盘查询）
2. 检查日志确认 Redis 连接正常

---

## 🔍 常见问题

### Q1: 构建报错 - "Cannot read properties of undefined (reading 'fsPath')"

**解决方案：** 后端 **Root Directory** 须为 **fenghua-backend**；勿在 backend/根 放含 `builds`/`routes`/`functions` 的 `vercel.json`；可开 **Force no build cache** 或把 **Build Command** 设为 `npm run build` 再试。

### Q1b: 构建报错 - "No entrypoint found. Searched for: src/main.*, ..."

**解决方案：** 表示在错误目录下找入口。**必须**在 **Settings → General → Root Directory** 中填写 **`fenghua-backend`**（不能留空或填根目录），保存后重新部署。

### Q1c: 构建报错 - "No Output Directory named 'public' found"

**解决方案：** 项目内已提供 `fenghua-backend/public/`（含 `robots.txt`）。若仍报错，到 **Settings → Build & Development Settings → Output Directory** 将该项**清空**（Nest 后端以 serverless 函数为输出，无需静态 Output Directory）。

### Q2: 构建失败 - "Cannot find module"

**解决方案：**
- 检查 `package.json` 中所有依赖是否已添加
- 确保 `node_modules` 已正确安装
- 检查 Vercel 构建日志

### Q3: 数据库连接失败

**解决方案：**
- 检查 `DATABASE_URL` 是否正确
- 确认数据库服务允许外部连接
- 检查 SSL 模式设置（Vercel 需要 `?sslmode=require`）

### Q4: Redis 连接失败

**解决方案：**
- 检查 `REDIS_URL` 是否正确
- 确认 Redis 服务允许外部连接
- 检查防火墙设置

### Q5: CORS 错误（如：Access to fetch ... has been blocked by CORS policy）

**解决方案：**
- 在**后端** Vercel 项目的 **Environment Variables** 中设置 **`FRONTEND_URL`** = 前端完整地址，如 `https://fenghua-crm-frontend.vercel.app`（与前端访问地址一致），保存后**重新部署后端**
- 多个前端地址可用英文逗号分隔

### Q6: 函数超时

**解决方案：**
- 增加函数超时时间（Vercel Pro 计划支持更长超时）
- 优化数据库查询
- 使用 Redis 缓存减少数据库查询

### Q7: 环境变量未生效

**解决方案：**
- 确认环境变量已添加到 Vercel 项目设置
- 重新部署项目（环境变量更改需要重新部署）
- 检查变量名是否正确（区分大小写）

### Q8: 前端无法连接到后端

**解决方案：**
- 检查 `VITE_BACKEND_URL` 是否正确
- 确认后端 URL 可访问
- 检查网络请求（浏览器开发者工具）

---

## 📝 部署检查清单

### 部署前

- [ ] 代码已推送到 GitHub
- [ ] 所有依赖已添加到 `package.json`
- [ ] 本地测试通过
- [ ] 数据库服务已创建并配置
- [ ] Redis 服务已创建并配置

### 部署时

- [ ] 前端项目已创建在 Vercel
- [ ] 后端项目已创建在 Vercel
- [ ] 所有环境变量已配置
- [ ] 构建成功
- [ ] 部署成功

### 部署后

- [ ] 前端页面正常加载
- [ ] 后端 API 正常响应
- [ ] 数据库连接正常
- [ ] Redis 连接正常
- [ ] 登录功能正常
- [ ] 数据操作正常

---

## 🔗 相关资源

- [Vercel 文档](https://vercel.com/docs)
- [NestJS Serverless 部署](https://docs.nestjs.com/faq/serverless)
- [Vite 部署指南](https://vitejs.dev/guide/static-deploy.html)
- [Supabase 文档](https://supabase.com/docs)
- [Upstash Redis 文档](https://docs.upstash.com/redis)

---

## 📞 支持

如遇到问题，请：
1. 查看 Vercel 构建日志
2. 检查函数日志
3. 查看浏览器控制台错误
4. 联系技术支持

---

**部署文档版本：** 1.0  
**最后更新：** 2026-01-14
