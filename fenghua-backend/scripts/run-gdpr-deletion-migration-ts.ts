/**
 * Run GDPR Deletion Table Migration
 * 
 * This script runs the migration to create the gdpr_deletion_requests table
...
async function runMigration() {
  console.log('🚀 GDPR Deletion Table Migration\n');
  ...
  try {
    ...
    const tableCheck = await pool.query(
      `SELECT EXISTS (
        ...
        AND table_name = 'gdpr_deletion_requests'
      )`
    );
    ...
    if (tableExists) {
      console.log('⚠️  Table gdpr_deletion_requests already exists');
    ...
    // Read migration file
    const migrationPath = join(__dirname, '../migrations/032-add-gdpr-deletion-request-table.sql');
    ...
    const indexes = await pool.query(`
      SELECT indexname
      FROM pg_indexes
      WHERE tablename = 'gdpr_deletion_requests'
      ORDER BY indexname
    `);
